...
export default function StoresPage({ params: paramsPromise }: { params: Promise<{ locale: string }> }) {
  ...
  const fetchStores = async () => {
    try {
      ...
      // Instead of N+1 queries, we fetch counts in parallel or leave them as 0 
      // if the count queries fail, rather than crashing the whole page.
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingStore.id) {
        ...
        // ✅ Immediately update local state so new logo shows without waiting for fetchStores
          ...
          <div className="bg-white rounded-[20px] w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <ImageUpload 
                ...
                onChange={(url) => {
                  console.log('[DEBUG] ImageUpload onChange fired with URL:', url)
