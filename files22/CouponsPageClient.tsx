        ...
        {filtersOpen && (
          ...
          >
            <div className="bg-card border border-border rounded-2xl p-5 flex flex-wrap gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  ...
                  className={`w-9 h-5 rounded-full transition-colors relative ${
                    filters.verified ? 'bg-primary' : 'bg-border'
                  }`}
                >
                  <div
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  onClick={() => updateFilter('exclusive', !filters.exclusive)}
                  className={`w-9 h-5 rounded-full transition-colors relative ${
                    filters.exclusive ? 'bg-[#FF6B00]' : 'bg-border'
                  }`}
                >
                  <div
                    className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${
                      filters.exclusive ? 'translate-x-4' : 'translate-x-0.5'
                    }`}
                  />
                </div>
                <span className="flex items-center gap-1.5 text-[13px] font-semibold text-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                {(
                  [
                    ...
                    { val: 'fixed', label: 'AED Off' },
                  ] as const
                ).map((opt) => (
                  <button
                    key={opt.val}
                    onClick={() => updateFilter('discountType', opt.val)}
